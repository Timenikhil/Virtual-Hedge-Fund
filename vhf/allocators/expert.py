...
class EXPERT(IAllocator):
    """
    Mean-Variance Optimiser that replaces historical expected returns with
    EXPERT next-day forecasts, and blends EXPERT-implied per-asset volatility
    with the historical correlation matrix to form the covariance matrix.

    Covariance construction
    -----------------------
    Rather than running .cov() on raw EXPERT point forecasts (which converge to
    ...
    where
        ...
        D          : diag(σ_1, …, σ_n), per-asset annualised volatility
                     estimated from the std of the EXPERT multi-step forecast
                     *return* series

    This keeps correlation structure grounded in real data while letting EXPERT
    drive the scale of uncertainty for each asset individually.
    """

    def __init__(self, algo: str = "EXPERT", forecast_horizon: int = 30):
        super().__init__()
        self.prices = None
        self.algo = algo
    ...
    def _forecast_returns(self, model, last_price: float) -> np.ndarray:
        """
        Return a daily-return series implied by the EXPERT price forecast.
    ...
    def rebalance(
        ...
        try:
            ...
            for i, sid in enumerate(sids):
                ...
                # Falls back to historical vol when EXPERT forecasts are flat
